...
void Database::createDatabase()
{
    ...
    try {
        ...
        exec(R"(CREATE TABLE "contact" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `contact` INTEGER, `created_date` INTEGER, `last_activity_date` INTEGER, `name` TEXT NOT NULL, `gender` INTEGER DEFAULT 0, `type` INTEGER DEFAULT 0, `status` INTEGER DEFAULT 0, `notes` TEXT, `stars` INTEGER, `favourite` INTEGER DEFAULT 0, `address1` TEXT, `address2` TEXT, `postcode` TEXT, `city` TEXT, `region` TEXT, `state` TEXT, `country` TEXT, FOREIGN KEY(`contact`) REFERENCES `contact`(`id`) ON DELETE CASCADE ))");
        exec(R"(CREATE TABLE "channel" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `contact` INTEGER NOT NULL, `type` INTEGER NOT NULL DEFAULT 0, `value` TEXT, `verified` INTEGER NOT NULL DEFAULT 0, FOREIGN KEY(`contact`) REFERENCES `contact`(`id`) ON DELETE CASCADE ))");
        exec(R"(CREATE TABLE "intent" ( `id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, `contact` INTEGER NOT NULL, `type` INTEGER NOT NULL DEFAULT 0, `state` INTEGER NOT NULL DEFAULT 0, `abstract` TEXT, `notes` TEXT, FOREIGN KEY(`contact`) REFERENCES `contact`(`id`) ON DELETE CASCADE ))");
